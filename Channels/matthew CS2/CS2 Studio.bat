@echo off
setlocal
rem --- CS2 Studio launcher ---
rem Double-click to open the app. Or drag a video onto this file to open straight into Condense.
rem Keep this next to cs2_studio.py, condense_action.py and cs2_timeline.py.
cd /d "%~dp0"

if not exist "cs2_studio.py" (
  echo cs2_studio.py is not in this folder.
  echo Put CS2 Studio.bat in the SAME folder as cs2_studio.py, then try again.
  pause
  exit /b 1
)
...
if errorlevel 1 (
  echo.
  echo CS2 Studio could not start. Make sure Python is installed ^(python --version^).
